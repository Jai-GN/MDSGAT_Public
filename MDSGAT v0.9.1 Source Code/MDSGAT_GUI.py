...
def CheckConfig():
    ...
    if os.path.isfile(Filepath) != True:
        ...
        Config.close
        MainApp.PrintOutputs("\n[WARN] No previous configuraiton file detected. Please select desired configuration in the settings menu prior to creating a simulation setup script!\n")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                MainApp.PrintOutputs("No suitable text editor found. Please manually configure 'MDSGAT/MDS_Configuration.json' within the users appdata directory.")

    return

def GetConfig(Filepath):
     with open(Filepath, "r") as Config_File:
        Configuration_File = json.load(Config_File)
...
class ScriptWindow(customtkinter.CTkToplevel):
                ...
                def GenerateScriptButton(self):
                    ...
                    try:
                        ...
                        MainApp.PrintOutputs("")
                        Config_File = GetConfig(CheckConfig())
                        ...
                        if TranslatedInputs["Error Check"] == "Passed":
                            ScriptFile = SG.MDSScriptGen(TranslatedInputs["Script Settings"], TranslatedInputs["MDS Options"], TranslatedInputs["Hardware Options"], TranslatedInputs["Advanced Options"],
                                                        TranslatedInputs["ION MDP"], TranslatedInputs["MIN MDP"], TranslatedInputs["NVT MDP"], TranslatedInputs["NPT MDP"], TranslatedInputs["PROD MDP" ], Config_File)
                            ...
                            if Config_File["Selected Configuration"]["Default Build"] == "NO BUILD SELECTED":
                                    MainApp.PrintOutputs("\n[WARN] Default configuration values detected! Either adjust this through the settings menu and recreate this script or manually edit the PBS file generated.")
...
class AnalysisWindow(customtkinter.CTkToplevel):
                def __init__(self, *args, **kwargs):
                    ...
                    self.SASA = customtkinter.CTkCheckBox(self.AnalysisChecksFrame,text="SASA",font=('arial',11),state='disabled')
                    ...
                    self.ComparativeSASA = customtkinter.CTkCheckBox(self.AnalysisChecksFrame,text="Comparative SASA",font=('arial',11),state='disabled')
                    self.ComparativeSASA.grid(row=5,column=0, pady=5)

                    self.ComparativeHBonds = customtkinter.CTkCheckBox(self.AnalysisChecksFrame,text="Comparative H-Bonds",font=('arial',11),state='disabled')
...
class SettingsWindow(customtkinter.CTkToplevel):
                def __init__(self, *args, **kwargs):
                    ...
                    self.Header = customtkinter.CTkLabel(self, text="MDSGAT Settings", fg_color = "transparent", font=('arial',20),anchor=("center"))
                    self.Header.grid(row=0,column=0,columnspan=2, pady=(20,0),sticky="nsew")

                    self.SettingsTabs = customtkinter.CTkTabview(self,width=450, height=650)
                    self.SettingsTabs.grid(row=1,column=0,columnspan=2,padx=20,pady=(0,5),sticky="nsew")
                    self.SettingsTabs.columnconfigure((0,0),weight=1)
                    self.SettingsTabs.add("Saved Organisations")
                    #self.SettingsTabs.add("Add Organisation")
                    self.Config_File = GetConfig(CheckConfig())

                    self.OrganisationSelectFrame = customtkinter.CTkFrame(self.SettingsTabs.tab("Saved Organisations"))
                    self.OrganisationSelectFrame.grid(row=0,column=0,columnspan=2,sticky='nsew')
                    self.OrganisationSelectFrame.columnconfigure((0,0),weight=1)
                    self.OrganisationSelectFrame.rowconfigure((1,1), weight=1)

                    self.OrganisationLabel = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Select Organisation: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.OrganisationLabel.grid(row=0,column=0,pady=(5,5),sticky="ew")

                    Organisations = []
                    for i in range(len(self.Config_File["Organisations"])):
                         Organisations.append(self.Config_File["Organisations"][i]["Organisation Name"])
                    self.OrganisationList = customtkinter.CTkComboBox(self.OrganisationSelectFrame, values=Organisations,command=self.OrganisationSelection)
                    self.OrganisationList.grid(row=0,column=1,pady=(5,5),sticky="nsew")

                    self.SystemLabel = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Select System: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.SystemLabel.grid(row=1,column=0,pady=(5,5),sticky="ew")
                    self.SystemList = customtkinter.CTkComboBox(self.OrganisationSelectFrame, values=[],command=self.SystemSelection,state="disabled")
                    self.SystemList.grid(row=1,column=1,pady=(5,5),sticky="nsew")

                    self.ProjectLabel = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Select Project: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.ProjectLabel.grid(row=2,column=0,pady=(5,5),sticky="ew")
                    self.ProjectList = customtkinter.CTkComboBox(self.OrganisationSelectFrame, values=[],command=self.ProjectSelection,state="disabled")
                    self.ProjectList.grid(row=2,column=1,pady=(5,5),sticky="nsew")

                    self.ConfigSpecificationsHeader = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Configuration Specifications",fg_color="transparent",font=('arial',24),anchor='center')
                    self.ConfigSpecificationsHeader.grid(row=4,column=0,columnspan=2,pady=(5,5),sticky="we")

                    self.OrganisationSpec = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Selected Organisation: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.OrganisationSpec.grid(row=5,column=0,pady=(5,5),sticky="e")
                    self.OrganisationSpecSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.OrganisationSpecSelection.grid(row=5,column=1,pady=(5,5),padx=(20,10),sticky="nsew")

                    self.SystemSpec = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Selected System: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.SystemSpec.grid(row=6,column=0,pady=(5,5),sticky="e")
                    self.SystemSpecSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, width=200, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.SystemSpecSelection.grid(row=6,column=1,pady=(5,5),padx=(20,10),sticky="nsew")

                    self.ProjectSpec = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Selected Project: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.ProjectSpec.grid(row=7,column=0,pady=(5,5),sticky="e")
                    self.ProjectSpecSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, width=200, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.ProjectSpecSelection.grid(row=7,column=1,pady=(5,5),padx=(20,10),sticky="nsew")

                    self.DefaultBuild = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="Default Build Path: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.DefaultBuild.grid(row=8,column=0,pady=(25,5),sticky="e")
                    self.DefaultBuildSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, width=200, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.DefaultBuildSelection.grid(row=8,column=1,pady=(25,5),padx=(20,10),sticky="nsew")

                    self.GPUBuild = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="GPU Build Path: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.GPUBuild.grid(row=9,column=0,pady=(5,5),sticky="e")
                    self.GPUBuildSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, width=200, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.GPUBuildSelection.grid(row=9,column=1,pady=(5,5),padx=(20,10),sticky="nsew")

                    self.MPIBuild = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="MPI Build Path: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.MPIBuild.grid(row=10,column=0,pady=(5,5),sticky="e")
                    self.MPIBuildSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, width=200, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.MPIBuildSelection.grid(row=10,column=1,pady=(5,5),padx=(20,10),sticky="nsew")

                    self.GPUMPIBuild = customtkinter.CTkLabel(self.OrganisationSelectFrame, text="GPU + MPI Build Path: ",fg_color="transparent",font=('arial',16),anchor='w')
                    self.GPUMPIBuild.grid(row=11,column=0,pady=(5,5),sticky="e")
                    self.GPUMPIBuildSelection = customtkinter.CTkLabel(self.OrganisationSelectFrame, width=200, text="",fg_color="transparent",font=('arial',12),anchor='w')
                    self.GPUMPIBuildSelection.grid(row=11,column=1,pady=(5,5),padx=(20,10),sticky="nsew")


                    self.EditSettingsButton = customtkinter.CTkButton(self.OrganisationSelectFrame,text="Edit Selection",state='disabled',command=self.EditSettings,width=200, font=('arial',16),anchor='center')
                    self.EditSettingsButton.grid(row=12,column=0,columnspan=2,sticky='ew',pady=(5,5))

                    self.UpdateSettingsButton = customtkinter.CTkButton(self.OrganisationSelectFrame,text="Confirm Selection",state='disabled',command=self.UpdateSettings,width=425, font=('arial',16),anchor='center')
                    self.UpdateSettingsButton.grid(row=13,column=0,columnspan=2,sticky='w',pady=(5,5))



                def OrganisationSelection(self, option):
                     Systems = []
                     for i in range(len(self.Config_File["Organisations"])):
                          if self.Config_File["Organisations"][i]["Organisation Name"] == self.OrganisationList.get():
                               self.OrganisationIndex = i
                               for c in range(len(self.Config_File["Organisations"][i]["Systems"])):
                                    Systems.append((self.Config_File["Organisations"][i])["Systems"][c]["System Name"])
                     self.SystemList.configure(state="normal", values = Systems)

                def SystemSelection(self, option):
                     Projects = []
                     for i in range(len((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"])):
                          if (self.Config_File["Organisations"][self.OrganisationIndex]["Systems"][i]["System Name"]) == self.SystemList.get():
                               self.SystemIndex = i
                               for c in range(len(self.Config_File["Organisations"][self.OrganisationIndex]["Systems"][i]["Projects"])):
                                    Projects.append(self.Config_File["Organisations"][self.OrganisationIndex]["Systems"][i]["Projects"][c])
                     self.ProjectList.configure(state='normal', values = Projects)

                def ProjectSelection(self, option):
                     for i in range(len(((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["Projects"])):
                          if ((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["Projects"][i] == self.ProjectList.get():
                               self.ProjectIndex = i
                               
                     self.OrganisationSpecSelection.configure(text=self.OrganisationList.get())
                     self.SystemSpecSelection.configure(text=self.SystemList.get())
                     self.ProjectSpecSelection.configure(text=self.ProjectList.get())
                     self.DefaultBuildSelection.configure(text=((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["Default Build"])
                     self.GPUBuildSelection.configure(text=((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["GPU Build"])
                     self.MPIBuildSelection.configure(text=((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["MPI Build"])
                     self.GPUMPIBuildSelection.configure(text=((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["MPI + GPU Build"])
                     self.EditSettingsButton.configure(state='normal')
                     self.UpdateSettingsButton.configure(state='normal')


                def EditSettings(self):
                     OpenConfig(CheckConfig())
                     MainApp.PrintOutputs("\n[NOTE] Please ensure all changes to the configuration file are saved before re-opening the settings menu to ensure that changes are reflected!\n")
                     self.destroy()


                def UpdateSettings(self):
                     self.Config_File["Selected Configuration"]["Organisation Name"] = self.OrganisationList.get()
                     self.Config_File["Selected Configuration"]["System Name"] = self.SystemList.get()
                     self.Config_File["Selected Configuration"]["Project Name"] = self.ProjectList.get()
                     self.Config_File["Selected Configuration"]["Default Build"] = ((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["Default Build"]
                     self.Config_File["Selected Configuration"]["GPU Build"] = ((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["GPU Build"]
                     self.Config_File["Selected Configuration"]["MPI Build"] = ((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["MPI Build"]
                     self.Config_File["Selected Configuration"]["MPI + GPU Build"] = ((self.Config_File["Organisations"][self.OrganisationIndex])["Systems"][self.SystemIndex])["MPI + GPU Build"]

                     with open(CheckConfig(), 'w') as configuration_file:
                          json.dump(self.Config_File,configuration_file,indent=4)
                          MainApp.PrintOutputs("Setting saved successfully!")
...
class MainWindow(customtkinter.CTk):
    def __init__(self):
        ...
        ToolTip(self.SettingsButton, msg="Opens the MDSGAT Settings Window", delay = 1.0)
        self.OpenConfiguration = None
    ...
    def OpenAnalysisWindow(self):
        if self.OpenAnalysisWindow is None or not self.OpenAnalysisWindow.winfo_exists():
            ...
            self.PrintOutputs("Opening MDS Analysis")
        else:
             self.OpenAnalysisWindow.focus()

    # Settings #
    def OpenConfiguration(self):
        if self.OpenConfiguration is None or not self.OpenConfiguration.winfo_exists():
            self.OpenConfiguration = SettingsWindow(self)
            self.PrintOutputs("Opening MDSGAT Settings")
            CheckConfig()
        else:
             self.OpenConfiguration.focus()
        #OpenConfig(CheckConfig())
